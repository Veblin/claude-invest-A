...
- **tushare hk_daily**：⚠️ **间歇性空返回**——首调成功（4 行）后同参数重调 0 行，疑限频/权限抖动；作交叉源使用时以非空为准，勿判死。
...
- **tushare 权限实锤（绕过 TushareClient 直打 HTTP 的 code/msg）**：
  - `hk_basic` ✅ code=0（1 行）。
  - **`hk_daily` = code 40203 频率超限「1 次/分钟」**——修正 §7「间歇性空返回/权限抖动」的表述：真实原因不是抖动，是**硬限频 1 次/分钟**（同一分钟内重复调用必空返回）。作交叉源时每次调用前须距上次 ≥60s。
